<template>
	<div class="itable">
		<el-table
			:data="tableData"
			:border="border"
			@selection-change="selectChangeHandler"
			style="width: 100%;">
			<el-table-column
				v-for="(col,index) in tableModel"
				:prop="col.prop"
				:label="col.label" 
				:align="col.align"
				:key="col.prop" 
				:width="col.width" 
				:formatter="col.formatter" 
				v-if="col.render == undefined && col.type == undefined">
			</el-table-column>
			<el-table-column :align="col.align" v-else-if="col.type != undefined && col.render == undefined" :type="col.type" :width="col.width" :selectable="col.selectable"></el-table-column>
			<el-table-column  
				v-else-if="col.type == undefined && col.render != undefined" 
				:label="col.label"  
				:id="col.prop" 
				:align="col.align"
				:width="col.width" 
				:key="col.prop" >
				<template slot-scope='scope'>
					<v-render  :render="col.render" :row="scope"  :column="scope.row.column" :index="scope.row.$index" ></v-render>
				</template>
			</el-table-column>
		</el-table>
	</div>
</template>

<script>
	import render from './expand'
	export default {
		name:'itable',
		props:{
			tableData:{
				type:Array,
				default(){
					return []
				}
			},
			tableModel:{
				type:Array,
				default(){
					return []
				}
			},
			border:{
				type:Boolean,
				default (){
					return false
				}
			},
			selectionChange:{
				type:Function,
				default(){
					return ''
				}
			}
		},
		created(){
		},
		methods:{
			selectChangeHandler(selections){
				this.$emit('selectionChange',selections);
			}
		},
		components:{
			'v-render':render
		}
	}
</script>

<style>
</style>